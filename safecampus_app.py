...
def load_models():
    ...
    model_path = os.path.join(model_dir, f"{meta['best_model_name']}.pkl")

    tfidf = joblib.load(tfidf_path)
    model = joblib.load(model_path)

    tfidf_multi, lr_multi = None, None
    if os.path.exists(os.path.join(model_dir, "tfidf_multi.pkl")):
        tfidf_multi = joblib.load(os.path.join(model_dir, "tfidf_multi.pkl"))
        lr_multi    = joblib.load(os.path.join(model_dir, "lr_multi.pkl"))

    return tfidf, model, tfidf_multi, lr_multi, meta

# ── Full prediction ──────────────────────────────────────────────────────────
def predict_full(text, tfidf, model, tfidf_multi=None, lr_multi=None, meta=None):
    ...
    bully_type = None
    if tfidf_multi and lr_multi and meta and "multi_classes" in meta:
        vec_m      = tfidf_multi.transform([cleaned])
        idx        = lr_multi.predict(vec_m)[0]
        bully_type = meta["multi_classes"][idx]
...
# ── Load models ───────────────────────────────────────────────────────────────
tfidf, model, tfidf_multi, lr_multi, meta = load_models()

models_ok = tfidf is not None and model is not None
...
if page == "📋 Report Incident":
    ...
    if analyze_btn and message_input.strip():
        with st.spinner("Analyzing message..."):
            ...
            result = predict_full(message_input, tfidf, model,
                                  tfidf_multi, lr_multi, meta)
        ...
        if show_lime:
            with st.expander("🔬 Word-level explanation (LIME)", expanded=True):
                ...
                with st.spinner("Running LIME explanation..."):
                    lime_words = get_lime_words(message_input, tfidf, model)
                if lime_words:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if btn and text_input.strip():
        result = predict_full(text_input, tfidf, model, tfidf_multi, lr_multi, meta)
        ...
        for ex in examples:
            r = predict_full(ex, tfidf, model, tfidf_multi, lr_multi, meta)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if not models_ok:
        ...
        st.markdown(f"**Features:** TF-IDF (10,000 unigrams + bigrams)")
        st.markdown(f"**Dataset:** Kaggle Cyberbullying Classification Dataset (~47,000 tweets)")

        c1, c2, c3 = st.columns(3)
        c1.metric("F1 Score",  meta.get("f1", "—"))
        c2.metric("AUC-ROC",   meta.get("auc", "—"))
        c3.metric("Accuracy",  meta.get("accuracy", "—"))
